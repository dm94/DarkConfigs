...
export const oldFeatures = ["com.deeme.modules.PallladiumHangar", "eu.darkbot.drmalito.minicore.MiniTrain", "com.deeme.modules.PaladiumModule", "eu.darkbot.leanon00.Main.Features.GGSpinner", "com.deeme.tasks.Discord"];
...
export const cleanConfig = (config: ConfigFile): ConfigFile => {
    ...
    configCopy["UNRESOLVED"] = undefined;
...
export const cleanDisabledFeatures = (config: ConfigFile): ConfigFile => {
    ...
    customConfigsKeys.filter((key) => enableFeatures.includes(key)).forEach((key) => {
...
export const cleanLeakInfoFromFeatures = (featureKey: string, feature: any): any => {
